...
with st.sidebar:
    ...
    rho = st.number_input("Density (ρ) in kg/m³", min_value=1e-9, value=1.225, format="%.4f")
    mu = st.number_input("Dynamic Viscosity (μ) in Pa-s", min_value=1e-9, value=1.81e-5, format="%.2e")
...
if app_choice in [1, 2]:
    ...
    with col1:
        ...
        if cross_sec_choice == 1: # Circular
            diam = st.number_input("Diameter (m)", min_value=1e-9, value=1.0, format="%.4f")
            Dh = diam
            A = (math.pi / 4) * diam**2
        elif cross_sec_choice == 2: # Annular
            Din = st.number_input("Inner Diameter (m)", min_value=1e-9, value=0.5, format="%.4f")
            Dout = st.number_input("Outer Diameter (m)", min_value=1e-9, value=1.0, format="%.4f")
            if Dout > Din:
                Dh = Dout - Din
            ...
            side = st.number_input("Side Length (m)", min_value=1e-9, value=1.0, format="%.4f")
            Dh = side
            A = side**2
        elif cross_sec_choice == 4: # Rectangular
            a = st.number_input("Width (m)", min_value=1e-9, value=2.0, format="%.4f")
            b = st.number_input("Height (m)", min_value=1e-9, value=1.0, format="%.4f")
            Dh = 2 * a * b / (a + b)
            A = a * b
        elif cross_sec_choice == 5: # 2D Channel
            a = st.number_input("Channel Height (m)", min_value=1e-9, value=1.0, format="%.4f")
            Dh = 2 * a
            A = None
            is_2d_channel = True
        elif cross_sec_choice == 6: # Other
            A_in = st.number_input("Cross-sectional Area (m²)", min_value=1e-9, value=1.0, format="%.4f")
            P_in = st.number_input("Wetted Perimeter (m)", min_value=1e-9, value=4.0, format="%.4f")
            Dh = 4 * A_in / P_in
            A = A_in
        elif cross_sec_choice == 7: # Specified Dh
            Dh = st.number_input("Hydraulic Diameter (m)", min_value=1e-9, value=1.0, format="%.4f")
            A = (math.pi / 4) * Dh**2
    ...
    with col2:
        ...
        if is_2d_channel:
            ...
            U_in = st.number_input("Inlet Velocity (m/s)", min_value=1e-9, value=10.0, format="%.4f")
            U = U_in
        else:
            ...
            if vel_type_choice == 1:
                U_in = st.number_input("Velocity (m/s)", min_value=1e-9, value=10.0, format="%.4f")
                U = U_in
            elif vel_type_choice == 2:
                mDot = st.number_input("Mass Flow Rate (kg/s)", min_value=1e-9, value=1.0, format="%.4f")
                U = mDot / (rho * A)
            elif vel_type_choice == 3:
                QDot = st.number_input("Volume Flow Rate (m³/s)", min_value=1e-9, value=1.0, format="%.4f")
                U = QDot / A
    ...
    if l_choice == 1:
        ...
        if delta_choice == "Estimate for fully developed flow":
            ...
            delta_99 = st.number_input("Boundary Layer Thickness (m)", min_value=1e-9, value=0.1, format="%.4f")
        l = 0.4 * delta_99
    elif l_choice == 3:
        l_in = st.number_input("Characteristic Length (m)", min_value=1e-9, value=0.01, format="%.4f")
        l = l_in
...
# --- Case 2: External / General Flows ---
    ...
    if model_choice > 1:
        ...
        U = st.number_input("Reference Velocity (U_ref) in m/s", min_value=1e-9, value=10.0, format="%.4f")
